...
void DrawRectangle(int x, int y, int w, int h, float* dwColour)
{
    ...
    {
        {(float) x, (float) y+h, 0.0f, 0.5, dwColour[0], dwColour[1], dwColour[2], dwColour[3]},
        {(float) x, (float) y, 0.0f, 0.5, dwColour[0], dwColour[1], dwColour[2], dwColour[3]},
		{(float) x+w, (float) y+h, 0.0f, 0.5, dwColour[0], dwColour[1], dwColour[2], dwColour[3]},
        {(float) x+w, (float) y, 0.0f, 0.5,  dwColour[0], dwColour[1], dwColour[2], dwColour[3]},
    };
    glBegin(GL_TRIANGLE_STRIP);
    for (size_t j=0;j<4;++j)
    {
      glColor4f(cvVertices[j].r/255.0, cvVertices[j].g/255.0, cvVertices[j].b/255.0, cvVertices[j].a/255.0);
